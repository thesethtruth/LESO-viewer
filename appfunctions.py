...
def make_profile_plot(startingweek, data):
    ...
    fig.update_layout(
        title="Energy balance in <b>week {startingweek}</b>".format(
...
def make_capex_pie(data):
    ...
    CAPEX = {ckey: calc_capex(ckey) for ckey in ckeys if calc_capex(ckey) > 1}
    ...
    fig.update_layout(legend_orientation="h", margin=global_margins)
...
def make_energy_over_year(data):
        ...
        for column in df.columns:
            fig.add_trace(
                go.Scatter(
                    ...
                    stackgroup="one",
                    groupnorm="percent",
                    line=dict(width=1, color=extract_color(column, power)),
                ),
                row,
                1,
            )
    ...
    tag = "power [+]"
    powers = {ckey: getter(ckey, tag) for ckey in ckeys if summer(ckey, tag) > 1}
    ...
    tag = "power [-]"
    loads = {ckey: getter(ckey, tag) for ckey in ckeys if summer(ckey, tag) < -1}
...
figure_2_options = {
    "energy_pie_chart": {
        "function": make_energy_pie,
        "display-text": "Yearly energy supply/demand by source",
    },
    "capex_pie_chart": {
        "function": make_capex_pie,
        "display-text": "CAPEX per component",
    },
    "energy_per_month": {
        "function": make_energy_over_year,
        "display-text": "Monthly energy supply/demand by source",
    },
}
